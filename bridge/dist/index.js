...
app.post('/slack/backfill', async (_req, res) => {
    try {
        const imported = await slackBackfill();
...
app.get('/google/install', async (_req, res) => {
    try {
        const url = await beginGoogleInstall();
...
app.get('/google/calendars', async (_req, res) => {
    try {
        res.json({ ok: true, calendars: await listCalendars() });
    }
    catch (e) {
        res.status(500).json({ ok: false, error: e.message });
    }
...
app.post('/google/backfill', async (_req, res) => {
    try {
        const imported = await backfillCalendar();
        res.json({ ok: true, imported });
    }
    catch (e) {
        res.status(500).json({ ok: false, error: e.message });
